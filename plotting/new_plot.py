...
def plot_lat_comparsion_w_breakdown_split(df: pd.DataFrame, args: EvalArgs):
    ...
    for i, (rect, task_name) in enumerate(zip(bar2, default_df["task_name"])):
        ...
        ax.text(rect.get_x() + rect.get_width() / 2.0, height, f'{score*100:.2f}%', ha='center', va='bottom', fontsize=7)
...
def plot_vary_alpha(df: pd.DataFrame, args: EvalArgs):
    ...
    for i, task_name in enumerate(TASKS):
        ...
        alphas = [0.0, 0.01, 0.02, 0.05, 0.1, 0.3, 0.5, 0.7, 0.9, 1.0]
...
def plot_vary_alpha_beta(df: pd.DataFrame, args: EvalArgs):
    selected_df = []
    for task_name in TASKS:
        df_tmp = df[df["task_name"] == task_name]
        df_tmp = shared_filter(df_tmp, task_name, args)
    ...
    for i, task_name in enumerate(TASKS):
        ...
        for beta, color in zip(all_betas, all_colors): 
            ...
            twnx = axes[i].twinx()
            twnx.scatter(df_tmp_beta["alpha"], df_tmp_beta[args.score_type], marker='+', color=color)
            twnx.plot(df_tmp_beta["alpha"], df_tmp_beta[args.score_type], marker='+', color=color, label=f"Accuracy")
        ...
        twnx.legend(loc="upper right")
    plt.tight_layout()
    plt.savefig(os.path.join(args.home_dir, args.plot_dir, "sim-sup_vary_alpha_beta.pdf"))
    # plt.show()
