...
def get_embedding(database: str, table: str, col: str) -> dict:
    ...
    df: pd.DataFrame = db_client.query_df(f'select distinct {col} from {database}.{table} order by {col}')
...
class CCFraudQ0(XIPQuery):
    ...
    def run(self, request: CCFraudRequest, qcfg: XIPQueryConfig) -> XIPFeatureVec:
        ...
        cat_fvals = np.array([self.embeddings[key].get(request[f'req_{key}'], 0) for key in self.cat_fnames])
...
class CCFraudQ1(XIPQuery):
    ...
    def run(self, request: CCFraudRequest, qcfg: XIPQueryConfig) -> XIPFeatureVec:
        ...
        for i in range(len(self.num_fnames), len(fvals)):
            fvals[i] = self.embeddings[self.cat_fnames[i - len(self.num_fnames)]][fvals[i]]
        fests = np.zeros_like(fvals)
        fdists = ['normal'] * len(fvals)
        return XIPFeatureVec(fnames=self.fnames, fvals=fvals, fests=fests, fdists=fdists)
...
class CCFraudQ4(XIPQuery):
    def __init__(self, key: str, data_loader: CCFraudTxnsLoader,
        ...
        wsize = data_loader.window_size
        col = 'is_fraud'
        fnames = [f'sum_{col}_{wsize}days',
                  f'avg_{col}_{wsize}days']
        if n_cfgs == 0:
            qsamples = [0.1, 0.5, 1.0]
        elif n_cfgs == -1:
            qsamples = [0.1, 1.0]
