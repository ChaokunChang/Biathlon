...
command = f"python run.py --example tickv2 --stage offline --task test/tickv2 --model {model} --nparts {nparts} --nreqs {offline_nreqs} --ncfgs {ncfgs} --clear_cache --ncores {ncores}"
print(command)
if args.nocache:
    os.system(command=command)


cmd_prefix = f"python run.py --example tickv2 --stage online --task test/tickv2 --model {model} --nparts {nparts} --offline_nreqs {offline_nreqs} --ncfgs {ncfgs} --ncores {ncores}"
path_prefix = f"/home/ckchang/.cache/apxinf/xip/test/tickv2/seed-0/online/{model}/ncores-{ncores}/ldnthreads-1/nparts-{nparts}"
...
evals.to_csv(
    f"./tickv2_{qinf}_{model}_{nparts}_{ncfgs}_{ncores}_{max_error}.csv", index=False
)
