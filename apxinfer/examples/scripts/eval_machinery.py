...
class EvalArgs(Tap):
    ...
    model: str = "mlp"
    max_error: float = 0.0
    agg_qids: list[int] = list(range(8))
    nocache: bool = False
    qinf: str = "sobol"
...
command = f"python run.py --example machinery --stage offline --task test/machinery --model {model} --nparts {nparts} --nreqs {offline_nreqs} --ncfgs {ncfgs} --clear_cache --ncores {ncores}"
print(command)
if args.nocache:
    os.system(command=command)


cmd_prefix = f"python run.py --example machinery --stage online --task test/machinery --model {model} --nparts {nparts} --offline_nreqs {offline_nreqs} --ncfgs {ncfgs} --ncores {ncores}"
path_prefix = f"/home/ckchang/.cache/apxinf/xip/test/machinery/seed-0/online/{model}/ncores-{ncores}/ldnthreads-1/nparts-{nparts}"
...
def extract_result(all_info: dict, min_conf, base_time=None):
    result = {
        ...
        "max_error": 0,
        "min_conf": min_conf,
        "sampling_rate": all_info["avg_sample"] / 8.0,
...
evals.to_csv(f"./machinery_{qinf}_{model}_{nparts}_{ncfgs}_{ncores}_{max_error}.csv", index=False)
