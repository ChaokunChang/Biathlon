...
command = f"python run.py --example tick --stage offline --task test/tick --model {model} --nparts {nparts} --nreqs {offline_nreqs} --ncfgs {ncfgs} --clear_cache --ncores {ncores}"
print(command)
if args.nocache:
    os.system(command=command)


cmd_prefix = f"python run.py --example tick --stage online --task test/tick --model {model} --nparts {nparts} --offline_nreqs {offline_nreqs} --ncfgs {ncfgs} --ncores {ncores}"
path_prefix = f"/home/ckchang/.cache/apxinf/xip/test/tick/seed-0/online/{model}/ncores-{ncores}/ldnthreads-1/nparts-{nparts}"
...
evals.to_csv(f"./tick_{qinf}_{model}_{nparts}_{ncfgs}_{ncores}_{max_error}.csv", index=False)
