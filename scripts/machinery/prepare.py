...
def get_all_file_names():
    normal_file_names = glob.glob(os.path.join(datadir, "normal", "normal", "*.csv"))
    imnormal_file_names_6g = glob.glob(
        os.path.join(datadir, "imbalance", "imbalance", "6g", "*.csv")
    )
    imnormal_file_names_10g = glob.glob(
        os.path.join(datadir, "imbalance", "imbalance", "10g", "*.csv")
    )
    imnormal_file_names_15g = glob.glob(
        os.path.join(datadir, "imbalance", "imbalance", "15g", "*.csv")
    )
    imnormal_file_names_20g = glob.glob(
        os.path.join(datadir, "imbalance", "imbalance", "20g", "*.csv")
    )
    imnormal_file_names_25g = glob.glob(
        os.path.join(datadir, "imbalance", "imbalance", "25g", "*.csv")
    )
    imnormal_file_names_30g = glob.glob(
        os.path.join(datadir, "imbalance", "imbalance", "30g", "*.csv")
    )
...
def load_data():
    ...
    normal_df = pd.concat(
        (
            pd.read_csv(f, names=filecols).assign(src=f, label=0)
            for f in normal_file_names
        ),
        ignore_index=True,
    )
    imnormal_df_6g = pd.concat(
        (
            pd.read_csv(f, names=filecols).assign(src=f, label=1)
            for f in imnormal_file_names_6g
        ),
        ignore_index=True,
    )
    imnormal_df_10g = pd.concat(
        (
            pd.read_csv(f, names=filecols).assign(src=f, label=2)
            for f in imnormal_file_names_10g
        ),
        ignore_index=True,
    )
    imnormal_df_15g = pd.concat(
        (
            pd.read_csv(f, names=filecols).assign(src=f, label=3)
            for f in imnormal_file_names_15g
        ),
        ignore_index=True,
    )
    imnormal_df_20g = pd.concat(
        (
            pd.read_csv(f, names=filecols).assign(src=f, label=4)
            for f in imnormal_file_names_20g
        ),
        ignore_index=True,
    )
    imnormal_df_25g = pd.concat(
        (
            pd.read_csv(f, names=filecols).assign(src=f, label=5)
            for f in imnormal_file_names_25g
        ),
        ignore_index=True,
    )
    imnormal_df_30g = pd.concat(
        (
            pd.read_csv(f, names=filecols).assign(src=f, label=6)
            for f in imnormal_file_names_30g
        ),
        ignore_index=True,
    )
...
def prepare_file2bid():
    ...
    df = pd.DataFrame(
        {
            "src": [f for f in all_file_names for _ in range(5)],
...
def prepare_db(database="machinery_more"):
    dbconn = clickhouse_connect.get_client(
        ...
        username="default",
        password="",
        session_id=f"session_{database}",
    )
...
def compute_features(df: pd.DataFrame, nsample: int = 0):
    ...
    if nsample > 0:
        df = (
            df.groupby(["src", "label", "seg_id"])[filecols]
            .head(nsample)
...
def prepare_features():
    ...
    # compute spectograms
    spec_df = (
        df.groupby(["src", "label", "seg_id"]).apply(compute_spectograms).reset_index()
    )
...
def prepare_feature_dataset(
    task="binary_classification", fcols=[f"{col}_mean" for col in filecols]
):
    ...
    test_requests.to_csv(os.path.join(jobdir, "test_requests.csv"), index=False)
...
def compute_permuation_importance(
    pipe: Pipeline, X: pd.DataFrame, y, random_state=0, use_pipe_feature=True
):
...
def _get_feature_importance(pipe: Pipeline, X, y):
    ...
    if isinstance(model, (LGBMRegressor, LGBMClassifier)):
        ...
        return compute_permuation_importance(
            pipe, X, y, random_state=0, use_pipe_feature=True
        )
...
def prepare_pipeline(
    ...
    fcols=[f"{col}_mean" for col in filecols],
    model_name="mlp",
) -> Pipeline:
    jobdir = os.path.join(work_dir, task)
    ...
    feature_importance_df.to_csv(
        os.path.join(jobdir, "feature_importances.csv"), index=False
    )

    # save train features and test features
    train_df[fcols].to_csv(os.path.join(jobdir, "train_features.csv"), index=False)
    test_df[fcols].to_csv(os.path.join(jobdir, "test_features.csv"), index=False)
...
if __name__ == "__main__":
    ...
    first_time = False
    if first_time:
        print("prepare file2bid ...")
        prepare_file2bid()
        print("Prepare features ......")
        prepare_features()
        print("prepare online database")
