...
def plot_qids(
    ...
    x_values = qid_list.astype(int)

    nround_list = np.array([len(res["history"]) for res in res_list])
    correct_list = np.array(
        [
            is_same_float(res["xip_pred"]["pred_value"], res["y_exact"])
            for res in res_list
        ]
    )
    ...
    nround_list = np.mean(nround_list.reshape(len(rid_list), len(qid_list)), axis=0)
    correct_list = np.mean(correct_list.reshape(len(rid_list), len(qid_list)), axis=0)
    avg_samples_list = np.mean(
        avg_samples_list.reshape(len(rid_list), len(qid_list)), axis=0
    )
    qsamples_list = np.mean(
        qsamples_list.reshape(len(rid_list), len(qid_list), nops), axis=0
    )
    ...
    plt.tight_layout()
    # save the plot
    save_dir = os.path.join(args.save_dir, "qid")
    os.makedirs(save_dir, exist_ok=True)
    tag = args.get_tag()
    save_path = os.path.join(save_dir, f"qid_{tag}.png")
    plt.savefig(save_path)
    plt.savefig(os.path.join(save_dir, "qid.png"))
    print(f"Saved vary qid plot to {save_path}")


if __name__ == "__main__":
    sim_args = simutils.SimulationArgs().parse_args()
    rid_list = np.arange(100)
    qid_list = np.array(simutils.task_meta[sim_args.task_name]["agg_ids"])
    res_list = []
    for rid in tqdm(rid_list, desc="rid", leave=False):
        for qid in tqdm(qid_list, desc="qid", leave=False):
            sim_args = simutils.SimulationArgs().parse_args()
            sim_args.rid = rid
            sim_args.selected_qid = qid
            res = simutils.run_default(sim_args)
            res_list.append(res)

    sim_args = simutils.SimulationArgs().parse_args()
    plot_qids(sim_args, rid_list, qid_list, res_list)
