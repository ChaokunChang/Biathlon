...
from apxinfer.examples.tick1000.data import TickRequest
from apxinfer.examples.tick1000.data import TickThisHourDataLoader, TickHourFStoreDataLoader
...
def get_embedding() -> dict:
    ...
    cpairs: pd.DataFrame = db_client.query_df(
        f"select distinct cpair from {tick_dbt} order by cpair"
    )
    currencies = []
...
class TickQP1(XIPQuery):
    def __init__(self, qname: str, enable_cache: bool = False, seed: int = 0) -> None:
        ...
        data_loader = TickThisHourDataLoader(
            "clickhouse", "xip", "tick1000", seed, enable_cache
        )
        self.dcols = ["bid"]
...
class TickQP2(XIPQuery):
    def __init__(
        ...
        data_loader = TickHourFStoreDataLoader(
            "clickhouse", "xip", "tick1000_fstore_hour", seed, enable_cache
        )
        self.offset = offset
        self.dcols = ["bid"]
        # self.dcol_aggs = [["avg", "stddevSamp"]]
    ...
    def run(self, request: TickRequest, qcfg: XIPQueryConfig, loading_nthreads: int = 1) -> XIPFeatureVec:
        ...
        fvals = self.data_loader.load_data(req, qcfg, cols=self.fstore_cols, loading_nthreads)
        return self.get_fvec_with_default_est(fvals=fvals)
