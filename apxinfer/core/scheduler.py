...
class XIPScheduler:
    ...
    def record(
        ...
        self.logger.debug(
            f'round-{len(self.history)}: qsamples = {[qcfg["qsample"] for qcfg in qcfgs]}'
        )
        self.logger.debug(
            f'round-{len(self.history)}: qcosts   = {[qcost["time"] for qcost in qcosts]}'
        )
        self.logger.debug(
            f'round-{len(self.history)}: pred={pred["pred_value"]}, error={pred["pred_error"]}, conf={pred["pred_conf"]}, {[qcost["time"] for qcost in qcosts]}'
        )
        self.history.append(
            XIPExecutionProfile(
    ...
    def get_next_qcfgs(
        ...
        max_num = np.sum(
            [
                len(self.fextractor.queries[i].cfg_pools) - 1 - qcfgs[i]["qcfg_id"]
...
class XIPAdvancedScheduler:
    """Advanced XIP Scheduler"""
    ...
    def __init__(
        ...
        self.init_qsamples = init_qsamples
        self.batch_size = batch_size
        self.verbose = verbose
    ...
    def get_init_qcfgs(self, request: XIPRequest) -> List[XIPQueryConfig]:
        """Get the initial set of qcfgs to start the scheduler"""
        if self.init_qsamples is not None:
            return [
                XIPQueryConfig(
                    qname=qry.qname,
                    qtype=qry.qtype,
                ...
                for qry, qsample in zip(self.fextractor.queries, self.init_qsamples)
            ]
        qcfgs = []
        for qry in self.fextractor.queries:
            if qry.qtype != XIPQType.AGG:
                qcfgs.append(qry.get_qcfg(0, 1.0, 0.0))
            else:
                qcfgs.append(qry.get_qcfg(0, 1.0 / self.max_nchunks, 0.0))
